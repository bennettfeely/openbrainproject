...
function slice() {
	...
	// var helper = new THREE.PlaneHelper(clip_plane[0], 20, 0xffff00);
	// scene.add(helper);
	// var helper = new THREE.PlaneHelper(clip_plane[1], 20, 0x00ffff);
	// scene.add(helper);

	stencilGroup = new THREE.Group();
	stencilGroup.rotation.set(Math.PI / 4, Math.PI / 4, 0);
	...
	backMat.stencilMask = 0x0f;
	backMat.stencilFail = THREE.KeepStencilOp;
	backMat.stencilZFail = THREE.KeepStencilOp;
	backMat.stencilZPass = THREE.DecrementWrapStencilOp;

	var backMesh = new THREE.Mesh(geometry, backMat);
	backMesh.renderOrder = 1;
	stencilGroup.add(backMesh);

	var intersectMat = new THREE.MeshBasicMaterial();
	intersectMat.color.set(0xff00fff);
	intersectMat.opacity = 0.5;
	intersectMat.transparent = true;
	intersectMat.stencilWrite = true;
	intersectMat.stencilFunc = THREE.NotEqualStencilFunc;
	intersectMat.stencilRef = 0;
	intersectMat.stencilMask = 0xff;
	intersectMat.stencilFail = THREE.ReplaceStencilOp;
	intersectMat.stencilZFail = THREE.ReplaceStencilOp;
	intersectMat.stencilZPass = THREE.ReplaceStencilOp;

	var intersectMesh = new THREE.Mesh(geometry, intersectMat);
	intersectMesh.renderOrder = 2;
	stencilGroup.add(intersectMesh);

	var displayMat = new THREE.MeshPhongMaterial();
	displayMat.opacity = 0.25;
	displayMat.transparent = true;

	var displayMesh = new THREE.Mesh(geometry, displayMat);
	displayMesh.renderOrder = 2;
	stencilGroup.add(displayMesh);

	var boxHelper = new THREE.BoxHelper(displayMesh, 0xffffff);
	boxHelper.renderOrder = 3;
	stencilGroup.add(boxHelper);
}
